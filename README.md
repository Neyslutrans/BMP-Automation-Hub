...
- `restore.py` - скрипт для восстановления репозитория.

### Основные скрипты

- `scripts` - папка, содержащая основные скрипты проекта.
  - `extra` - папка, содержащая скрипты обработки дополнительных выгрузок.
    - `affinity.py` - скрипт для обработки выгрузки "affinity".
